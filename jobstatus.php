...
<head>
    ...
    if($_GET["st"]==1)
    {
        $row=ongingjobs($_GET["jid"]);
        $result=$row->fetch_assoc();
    }elseif($_GET["st"]==2)
    {
        $row=completejobs($_GET["jid"]);
		$result=$row->fetch_assoc();
		$paymentst=paymentstatus($_GET["jid"]);
	}
	elseif($_GET["st"]==3)
    {
        $row=canceljobs($_GET["jid"]);
        $result=$row->fetch_assoc();
    }
									...
									<div class="wt-projectdetail-holder">
										...
										<div class="wt-skillsrequired">
                                            ...
                                            <div class="wt-userlistinghold wt-featured wt-userlistingvtwo">
												...
												<div class="wt-userlistingcontent">
													<div class="wt-contenthead">
                                                        ...
                                                        <a href="usersingle.php?uid=<?php echo $result["u_id"];?>"  class="wt-btn">VIEW Profile</a>
                                                        <a class="wt-btn">Hired</a>
													</div>   
												</div>	
											</div>
                                            <div class="wt-addnew form-group">
                                                <br><br>
                                                <?php if($_GET["st"]==1):?>
                                                    <a href="bidacction.php?com=<?php echo $_GET["jid"].",".$result["u_id"].",".$result["c_id"].",".$result["onid"]?>" class="wt-btn" style="color:#28a745;margin-left:5px;">Complete</a>   
								                    <a href="bidacction.php?can=<?php echo $_GET["jid"].",".$result["u_id"].",".$result["c_id"].",".$result["onid"]?>"class="wt-btn" style="color:red;">Cancel</a>
												<?elseif($_GET["st"]==2):?>
													<?php if(mysqli_num_rows($paymentst)>0):?>
														<a class="wt-btn">Payment completed</a>
													<?else:?>
														<a href="payment.php?jid=<? echo $result["jid"].",".$result["bdprice"].",".$result["uid"];?>" class="wt-btn" style="color:#28a745;margin-left:5px;">Pay</a>	
													<?endif;?>
                                                <?php endif?>
                                            </div>
									</div>
